...
% If Table > 0, values are written in white on the cells
...
if isempty(pp),
  p = TSPGreedyInsertion(D,[],40);        % put instances in a useful order
else
...
if Table > 0,
  for i = 1:s,
    for j = 1:s,
      ...
      text(i+0.5,j+0.5,tt,'HorizontalAlignment','Center','Color','w','fontsize',TFS);
    end
  end
end
...
if length(W) > 1,
  ...
  XFS = min(6,FS);
  for i = 1:t,
    text(i+0.5,t+1,SLab{p(i)},'Rotation',90,'HorizontalAlignment','right','FontSize',XFS,'FontName','FixedWidth');
  end
  set(gca,'XTickLabel',[])
end

set(gca,'YTick',(1:s)+0.5)
set(gca,'YTickLabel',SLab(p),'FontSize',FS,'FontName','FixedWidth')
