...
B = [B;[ 11    2.2  4.4 -7.5 -5.2 -3.0  3.0 -1.1 -0.7  260  -65 1.6]]; % cSs CLZ 2008-04-22. Moved up and right, angle moved right 2016-06-19 CLZ; expanded to accommodate exemplar 2016-12-19 CLZ
...
% GC pairs (paircode 7) ------------------------------------------------------
% Note:  Before 2016-11-11, the signs of the numbers for the categories were reversed, -5 instead of 5, and special code made it work out right.  That was crazy.
% After 2016-11-11, this should not be necessary, but it may take some time to work out all of the bugs.
% Categories 3, 4, 5, 6, 9, 11, 12 had their signs switched by 2016-12-17
...
B = [B;[ -3   -5.1 -3.5  4.0  6.4 -3.0  3.0  0.7  1.1   55   90 1.6]]; % cHW This is the LSW category 2016-07-16 CLZ
...
B = [B;[  5    6.4  8.0  3.5  5.5 -3.0  3.0 -1.1 -0.7    0   40 1.6]]; % cWS Moved down and right 2016-07-25 CLZ
% = [B;[ -5    4.2  5.2 -6.3 -5.3 -3.0  3.0 -1.1 -0.6   30   60 5.0]]; % cSW
B = [B;[ -5    4.0  5.3 -6.4 -5.3 -3.0  3.0 -1.1 -0.6  -10   10 1.6]]; % cSW Expanded 2016-07-16 CLZ
...
B = [B;[  8   -4.8 -3.0  6.8  7.9 -3.0  3.0  0.7  1.1  150  185 1.6]]; % tHH Tightened CLZ 2010-11-05
...
B = [B;[ -9    2.4  4.0 -6.6 -5.5 -3.0  3.0  0.7  1.1   -5   25 1.6]]; % cSH Instances found, expanded 2016-11-11 CLZ; renumbered from 9.1 to 9
B = [B;[ -9.1  3.6  5.2 -7.8 -6.6 -3.0  3.0  0.7  1.1  -25   -5 1.6]]; % cSH CLZ 2007-12-14; expanded down and left 2016-12-17 CLZ; renumbered 9 to 9.1
% = [B;[ 11    2.6  3.9 -7.1 -5.6 -3.0  3.0 -1.1 -0.7  250  -70 5.0]]; % cSs
B = [B;[ 11    1.7  3.4 -8.7 -6.3 -3.0  3.0 -1.1 -0.7  230  270 2.6]]; % cSs CLZ 2007-12-13
B = [B;[-11    5.5  7.4 -3.6 -1.9 -3.0  3.0 -1.1 -0.7  250  -60 2.5]]; % csS Shifted 2010-11-24 CLZ; hard to expand to add instances 2016-07-26
...
B = [B;[  6    0.0  4.0  7.7  9.4 -3.0  3.0  0.6  1.1   50   90 5.0]]; % tWS Expanded 2010-11-27 CLZ
...
B = [B;[  9   -7.0 -4.9  0.3  4.0 -3.0  3.0  0.7  1.1  -40    0 1.6]]; % cHS Expanded 2016-10-21 CLZ
...
B = [B;[  6   -0.7  2.7  7.8  9.6 -3.0  3.0  0.6  1.1   40   85 1.6]]; % tWS Exchanged with 6.1 CLZ 12-14-2007 - Jesse_4_18_08; exemplar accommodated 2016-12-19 CLZ
...
B = [B;[ -9.1  6.6  8.0 -4.5 -3.5 -3.0  3.0  0.7  1.1   40   80 5.0]]; % cHS Move to category -9.1 2016-11-10 CLZ; no instances
...
B = [B;[ -5    4.0  6.0 -5.0 -2.8 -3.0  3.0 -1.1 -0.7  -35    5 1.6]]; % cSW Instances found, limits adjusted 2016-11-11 CLZ; exemplar accommodated 2016-12-19 CLZ
...
B = [B;[ -9    4.0  6.6 -5.5 -3.1 -3.0  3.0  0.7  1.1    5   25 1.6]]; % cSH Moved up and right 2016-11-11 CLZ; expanded to accommodate exemplar 2016-12-19 CLZ
